...
# For each outer layer, we want to iterate through its elements in clockwise order starting from the top left corner. Suppose the current outer layer has top-left coordinates (r1, c1)\text{(r1, c1)}(r1, c1) and bottom-right coordinates (r2, c2)\text{(r2, c2)}(r2, c2).

# Then, the top row is the set of elements (r1, c)\text{(r1, c)}(r1, c) for c = c1,...,c2\text{c = c1,...,c2}c = c1,...,c2, in that order. The rest of the right side is the set of elements (r, c2)\text{(r, c2)}(r, c2) for r = r1+1,...,r2\text{r = r1+1,...,r2}r = r1+1,...,r2, in that order. Then, if there are four sides to this layer (ie., r1 < r2\text{r1 < r2}r1 < r2 and c1 < c2\text{c1 < c2}c1 < c2), we iterate through the bottom side and left side as shown in the solutions below.
...
#         O(1)O(1)O(1) without considering the output array, since we don't use any additional data structures for our computations.

#         O(N)O(N)O(N) if the output array is taken into account.
