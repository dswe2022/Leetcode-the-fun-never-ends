...
# We know that there could be 4 types of operations - addition (+), subtraction (-), multiplication
# (*) and division (/). Without parenthesis, we know that, multiplication (*) and (\) operations would 
# always have higher precedence than addition (+) and subtraction (-) based on operator precedence rules.
...
# In example 1, 4+3 is evaluated later because the next operation is multiplication (3*5) which has higher 
# precedence. But, in example 2, 4+3 is evaluated first because the next operation is subtraction (3-5) which 
...
#     Otherwise, the current character must be an operation (+,-,*, /). Evaluate the expression based on 
...
#     Stack data structure follows Last In First Out (LIFO) principle. Hence, the last pushed number in the 
#     stack would be popped out first for evaluation. In addition, when we pop from the stack and evaluate this 
#     expression in the future, we need a way to determine if the operation was Addition (+) or Subtraction (-). 
#     To simplify our evaluation, we can push -currentNumber in a stack if the current operation is subtraction (-) 
#     and assume that the operation for all the values in the stack is addition (+). This works because (a - currentNumber) is equivalent to (a + (-currentNumber)).
